...
class Auth {
  /** token
   * @description Creates a new token from the LMS API Authorization server and stores it on disk.
  ...
  async _token() {
    if (ls.getItem('fortunasdk-token')) {
      let _t = new Token(
        JSON.parse(Buffer.from(ls.getItem('fortunasdk-token'), 'base64').toString())
      );
      if (_t.isValid) {
        return _t;
      } else {
  ...
  async _createToken() {
    ...
    let data = Buffer.from(JSON.stringify(res)).toString('base64');
